...
from electrum_peseta.i18n import _, set_language
from electrum_peseta.util import print_error, print_msg
...
from electrum_peseta.pesetacoin import MIN_RELAY_TX_FEE, is_valid
from electrum_peseta.plugins import run_hook

import icons_rc

from electrum_peseta.util import format_satoshis, NotEnoughFunds
from electrum_peseta import Transaction
from electrum_peseta import mnemonic
from electrum_peseta import util, pesetacoin, commands, Interface, Wallet
from electrum_peseta import SimpleConfig, Wallet, WalletStorage
from electrum_peseta import Imported_Wallet
...
from electrum_peseta import ELECTRUM_VERSION
import re
...
class ElectrumWindow(QMainWindow):
    ...
    def __init__(self, config, network, gui_object):
        ...
        self.setWindowIcon(QIcon(":icons/electrum-peseta.png"))
        self.init_menubar()
    ...
    def load_wallet(self, wallet):
        import electrum_peseta as electrum
        self.wallet = wallet
        ...
        title = 'Electrum-PESETA ' + self.wallet.electrum_version + '  -  ' + os.path.basename(self.wallet.storage.path)
    ...
    def backup_wallet(self):
        ...
        if new_path != path:
            try:
                ...
                QMessageBox.critical(None,"Unable to create backup", _("Electrum-PESETA was unable to copy your wallet file to the specified location.")+"\n" + str(reason))
    ...
    def init_menubar(self):
        ...
        tools_menu.addAction(_("Electrum-PESETA preferences") if sys.platform == 'darwin' else _("Preferences"), self.settings_dialog)
        ...
        help_menu.addAction(_("&Official website"), lambda: webbrowser.open("http://electrum.pesetacoin.info"))
    ...
    def show_about(self):
        QMessageBox.about(self, "Electrum-PESETA",
            _("Version")+" %s" % (self.wallet.electrum_version) + "\n\n" + _("Electrum-PESETA's focus is speed, with low resource usage and simplifying Pesetacoin. You do not need to perform regular backups, because your wallet can be recovered from a secret phrase that you can memorize or write on paper. Startup times are instant because it operates in conjunction with high-performance servers that handle the most complicated parts of the Pesetacoin system."))

    def show_report_bug(self):
        QMessageBox.information(self, "Electrum-PESETA - " + _("Reporting Bugs"),
            _("Please report any bugs as issues on github:")+" <a href=\"https://github.com/electrumalt/electrum-peseta/issues\">https://github.com/electrumalt/electrum-peseta/issues</a>")
    ...
    def notify(self, message):
        if self.tray:
            self.tray.showMessage("Electrum-PESETA", message, QSystemTrayIcon.Information, 20000)
    ...
    def create_history_menu(self, position):
        ...
        be = self.config.get('block_explorer', 'live.pesetacoin.info')
        if be == 'live.pesetacoin.info':
            block_explorer = 'https://live.pesetacoin.info/tx/'
        elif be == 'explorer.pesetacoin.info':
            block_explorer = 'https://explorer.pesetacoin.info/tx/'
    ...
    def receive_at(self, addr):
        if not pesetacoin.is_address(addr):
            return
    ...
    def update_receive_qr(self):
        ...
        if addr:
            ...
            p = urlparse.ParseResult(scheme='pesetacoin', netloc='', path=addr, params='', query='&'.join(query), fragment='')
    ...
    def create_send_tab(self):
        ...
        self.payto_help = HelpButton(_('Recipient of the funds.') + '\n\n' + _('You may enter a Pesetacoin address, a label from your list of contacts (a list of completions will be proposed), or an alias (email-like address that forwards to a Pesetacoin address)'))
        ...
        msg = _('Pesetacoin transactions are in general not free. A transaction fee is paid by the sender of the funds.') + '\n\n'\
    ...
    def pay_from_URI(self,URI):
        ...
        try:
            ...
            QMessageBox.warning(self, _('Error'), _('Invalid pesetacoin URI:') + '\n' + str(e), _('OK'))
            return
        ...
        from electrum_peseta import paymentrequest
        def payment_request():
    ...
    def create_contact_menu(self, position):
        ...
        if not item:
            ...
            menu.addAction(_("QR code"), lambda: self.show_qrcode("pesetacoin:" + addr, _("Address")))
            if is_editable:
    ...
    def show_invoice(self, key):
        from electrum_peseta.paymentrequest import PaymentRequest
    ...
    def update_console(self):
        ...
        console.updateNamespace({'util' : util, 'pesetacoin':pesetacoin})
    ...
    def new_account_dialog(self, password):
        ...
        e = QLineEdit()
        vbox.addWidget(e)
        msg = _("Note: Newly created accounts are 'pending' until they receive pesetacoins.") + " " \
    ...
    def do_verify(self, address, message, signature):
        message = unicode(message.toPlainText())
        message = message.encode('utf-8')
        if pesetacoin.verify_message(address.text(), str(signature.toPlainText()), message):
    ...
    def do_encrypt(self, message_e, pubkey_e, encrypted_e):
        ...
        message = message.encode('utf-8')
        try:
            encrypted = pesetacoin.encrypt_message(message, str(pubkey_e.text()))
    ...
    def tx_from_text(self, txt):
        ...
        if is_hex:
            try:
                ...
                traceback.print_exc(file=sys.stdout)
                QMessageBox.critical(None, _("Unable to parse transaction"), _("Electrum-PESETA was unable to parse your transaction"))
                return
    ...
    def read_tx_from_qrcode(self):
        from electrum_peseta import qrscanner
        try:
        ...
        if not data:
            return
        # if the user scanned a pesetacoin URI
        if data.startswith("pesetacoin:"):
    ...
    def read_tx_from_file(self):
        ...
        try:
            ...
            QMessageBox.critical(None, _("Unable to read file or no transaction found"), _("Electrum-PESETA was unable to open your transaction file") + "\n" + str(reason))
    ...
    def do_process_from_csvReader(self, csvReader):
        ...
        try:
            for position, row in enumerate(csvReader):
                address = row[0]
                if not pesetacoin.is_address(address):
    ...
    def export_privkeys_dialog(self, password):
        ...
        defaultname = 'electrum-peseta-private-keys.csv'
        ...
        try:
            ...
            export_error_label = _("Electrum-PESETA was unable to produce a private key-export.")
    ...
    def do_import_labels(self):
        ...
        try:
            ...
            QMessageBox.critical(None, _("Unable to import labels"), _("Electrum-PESETA was unable to import your labels.")+"\n" + str(reason))


    def do_export_labels(self):
        labels = self.wallet.labels
        try:
            fileName = self.getSaveFileName(_("Select file to save your labels"), 'electrum_peseta_labels.dat', "*.dat")
            if fileName:
                with open(fileName, 'w+') as f:
                    json.dump(labels, f)
                QMessageBox.information(None, _("Labels exported"), _("Your labels where exported to")+" '%s'" % str(fileName))
        except (IOError, os.error), reason:
            QMessageBox.critical(None, _("Unable to export labels"), _("Electrum-PESETA was unable to export your labels.")+"\n" + str(reason))
    ...
    def export_history_dialog(self):
        ...
        defaultname = os.path.expanduser('~/electrum-peseta-history.csv')
        ...
        try:
            ...
            export_error_label = _("Electrum-PESETA was unable to produce a transaction export.")
    ...
    def settings_dialog(self):
        ...
        d = QDialog(self)
        d.setWindowTitle(_('Electrum-PESETA Settings'))
        d.setModal(1)
        vbox = QVBoxLayout()
        grid = QGridLayout()
        ...
        from electrum_peseta.i18n import languages
        ...
        fee_help = HelpButton(_('Fee per kilobyte of transaction.') + '\n' \
                              + _('Recommended value') + ': ' + self.format_amount(pesetacoin.RECOMMENDED_FEE) + ' ' + self.base_unit())
        ...
        block_explorers = ['live.pesetacoin.info', 'explorer.pesetacoin.info']
        block_ex_label = QLabel(_('Online Block Explorer') + ':')
        block_ex_combo = QComboBox()
        block_ex_combo.addItems(block_explorers)
        block_ex_combo.setCurrentIndex(block_explorers.index(self.config.get('block_explorer', 'explorer.pesetacoin.info')))
        ...
        if self.need_restart:
            QMessageBox.warning(self, _('Success'), _('Please restart Electrum-PESETA to activate the new GUI settings'), _('OK'))



    def run_network_dialog(self):
        if not self.network:
            QMessageBox.warning(self, _('Offline'), _('You are using Electrum-PESETA in offline mode.\nRestart Electrum-PESETA if you want to get connected.'), _('OK'))
            return
    ...
    def plugins_dialog(self):
        from electrum_peseta.plugins import plugins

        d = QDialog(self)
        d.setWindowTitle(_('Electrum-PESETA Plugins'))
        d.setModal(1)
