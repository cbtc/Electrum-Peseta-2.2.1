...
from electrum_peseta import pesetacoin, util
...
class Plugin(BasePlugin):
    ...
    def load_wallet(self, wallet):
        ...
        for key, xpub in mpk.items():
            keyname = key + '/' # fixme
            K = pesetacoin.deserialize_xkey(xpub)[-1].encode('hex')
            _hash = pesetacoin.Hash(K).encode('hex')
    ...
    def cosigner_can_sign(self, tx, cosigner_xpub):
        from electrum_peseta.transaction import x_to_xpub
        xpub_set = set([])
    ...
    def do_send(self, tx):
        for xpub, K, _hash in self.cosigner_list:
            if not self.cosigner_can_sign(tx, xpub):
                continue
            message = pesetacoin.encrypt_message(tx.raw, K)
            try:
    ...
    def on_receive(self):
        ...
        if not xprv:
            return
        try:
            k = pesetacoin.deserialize_xkey(xprv)[-1].encode('hex')
            EC = pesetacoin.EC_KEY(k.decode('hex'))
