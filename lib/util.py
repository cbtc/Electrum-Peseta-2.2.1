...
def user_dir():
    if "HOME" in os.environ:
        return os.path.join(os.environ["HOME"], ".electrum-peseta")
    elif "APPDATA" in os.environ:
        return os.path.join(os.environ["APPDATA"], "Electrum-PESETA")
    elif "LOCALAPPDATA" in os.environ:
        return os.path.join(os.environ["LOCALAPPDATA"], "Electrum-PESETA")
    elif 'ANDROID_DATA' in os.environ:
        return "/sdcard/electrum-peseta/"
    else:
...
def appdata_dir():
    """Find the path to the application data directory; add an electrum-peseta folder and return path."""
    if platform.system() == "Windows":
        return os.path.join(os.environ["APPDATA"], "Electrum-PESETA")
    elif platform.system() == "Linux":
        return os.path.join(sys.prefix, "share", "electrum-peseta")
    elif (platform.system() == "Darwin" or
          platform.system() == "DragonFly" or
          platform.system() == "OpenBSD" or
          platform.system() == "FreeBSD" or
	  platform.system() == "NetBSD"):
        return "/Library/Application Support/Electrum-PESETA"
    else:
...
def parse_URI(uri):
    ...
    if ':' not in uri:
        assert pesetacoin.is_address(uri)
        return uri, None, None, None, None

    u = urlparse.urlparse(uri)
    assert u.scheme == 'pesetacoin'

    address = u.path
    valid_address = pesetacoin.is_address(address)
