...
from pesetacoin import is_valid, hash_160_to_bc_address, hash_160
...
register_command('createmultisig',       2, 2, False, True,  False, 'similar to pesetacoind\'s command')
register_command('createrawtransaction', 2, 2, False, True,  False, 'Create an unsigned transaction. The syntax is similar to pesetacoind')
register_command('deseed',               0, 0, False, True,  False, 'Remove seed from wallet, creating a seedless, watching-only wallet.')
register_command('decoderawtransaction', 1, 1, False, False, False, 'similar to pesetacoind\'s command')
register_command('getprivatekeys',       1, 1, False, True,  True,  'Get the private keys of a given address', 'getprivatekeys <pesetacoin address>')
...
register_command('getpubkeys',           1, 1, False, True,  False, 'Return the public keys for a wallet address', 'getpubkeys <pesetacoin address>')
...
class Commands:
    ...
    def signtxwithkey(self, raw_tx, sec):
        ...
        pubkey = pesetacoin.public_key_from_private_key(sec)
        tx.sign({ pubkey:sec })
        return tx
    ...
    def getversion(self):
        import electrum_peseta as electrum  # Needs to stay here to prevent ciruclar imports
    ...
    def verifymessage(self, address, signature, message):
        return pesetacoin.verify_message(address, signature, message)

    def _mktx(self, outputs, fee = None, change_addr = None, domain = None):
        for to_address, amount in outputs:
            if not is_valid(to_address):
                raise Exception("Invalid Pesetacoin address", to_address)

        if change_addr:
            if not is_valid(change_addr):
                raise Exception("Invalid Pesetacoin address", change_addr)

        if domain is not None:
            for addr in domain:
                if not is_valid(addr):
                    raise Exception("invalid Pesetacoin address", addr)
    ...
    def encrypt(self, pubkey, message):
        return pesetacoin.encrypt_message(message, pubkey)
