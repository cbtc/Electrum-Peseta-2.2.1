...
class WalletStorage(object):
    ...
    def init_path(self, config):
        ...
        old_path = os.path.join(config.path, "electrum-peseta.dat")
...
class Abstract_Wallet(object):
    ...
    def get_addr_balance(self, address):
        'returns the confirmed balance and pending (unconfirmed) balance change of this pesetacoin address'
    ...
    def get_private_key_from_xpubkey(self, x_pubkey, password):
        if x_pubkey[0:2] in ['02','03','04']:
            addr = pesetacoin.public_key_to_bc_address(x_pubkey.decode('hex'))
...
class BIP32_Simple_Wallet(BIP32_Wallet):
    ...
    def create_xprv_wallet(self, xprv, password):
        xpub = pesetacoin.xpub_from_xprv(xprv)
...
class Wallet(object):
    ...
    def is_address(self, text):
        if not text:
            return False
        for x in text.split():
            if not pesetacoin.is_address(x):
                return False
        return True
    ...
    def is_private_key(self, text):
        if not text:
            return False
        for x in text.split():
            if not pesetacoin.is_private_key(x):
                return False
        return True
