...
def hash_160_to_bc_address(h160, addrtype=47):
...
def SecretToASecret(secret, compressed=False, addrtype=47):
    vchIn = chr((addrtype+128)&255) + secret
    if compressed: vchIn += '\01'
    return EncodeBase58Check(vchIn)

def ASecretToSecret(key, addrtype=47):
...
def msg_magic(message):
    ...
    return "\x18Pesetacoin Signed Message:\n" + encoded_varint + message
...
def _get_headers(testnet):
    """Returns the correct headers for either testnet or pesetacoin, in the form
...
def deserialize_xkey(xkey):
    ...
    # Determine if the key is a pesetacoin key or a testnet key.
